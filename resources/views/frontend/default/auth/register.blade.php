        ...
        <div class="row justify-content-center align-items-center">
            <div class="col-lg-6">
                <div class="register_form_iner">
                    <h2>{{ __('defaultTheme.welcome') }}! <br>{{ __('Please Create Account') }}</h2>
                        ...
                        @if(!empty($row) && !empty($form_data))
                            <div class="form-row">
                                ...
                                {{-- @foreach($form_data as $row)
                                    ...
                                    @elseif($row->type == 'text' || $row->type == 'number' || $row->type == 'email' || $row->type == 'date' || $row->type == 'password')
                                    ...
                                    @elseif($row->type == 'text')
                                        <div class="col-md-6 mb-10">
                                            <label for="phone"> {{$row->label}} @if($required) <span class="text-danger">*</span> @endif</label>
                                            <input {{$required ? 'required' :''}} type="{{$type}}" id="phone" class="@error($row->name) is-invalid @enderror" name="{{$row->name}}" value="{{ old($row->name) }}" placeholder="{{$placeholder}}">
                                            @error($row->name)
                                            <span class="text-danger" >{{ $message }}</span>
                                            @enderror
                                        </div>
                                    ...
                                    @elseif($row->type=='password')
                                        <div class="col-md-12 mb-10">
                                            <label for={{$row->name}}>{{$row->label}}@if($required) <span class="text-danger">*</span> @endif</label>
                                            <input {{$required ? 'required' :''}} type="{{$type}}" id="password" class="@error($row->name) is-invalid @enderror" name="{{$row->name}}" value="{{ old($row->name) }}" placeholder="{{$placeholder}}">
                                            <span class="text-danger">{{ $message }}</span>
                                        </div>
                        ...
                        @else
                            <div class="form-row">
                                ...
                                <div class="col-md-6 mb-10">
                                    ...
                                    <input type="text" id="first_name" class="@error('first_name') is-invalid @enderror" name="first_name" value="{{ old('first_name') }}" onfocus="this.placeholder = ''">
                                    @error('first_name')
                                    <span class="text-danger" >{{ $message }}</span>
                                    @enderror
                                </div>
                                <div class="col-md-6 mb-10">
                                    ...
                                    <input type="text" id="last_name" class="@error('last_name') is-invalid @enderror" name="last_name" value="{{ old('last_name') }}" onfocus="this.placeholder = ''"
                                          >
                                    @error('last_name')
                                    <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                ...
                                <div class="col-md-6 mb-10">
                                    @if(isModuleActive('Otp') && otp_configuration('otp_activation_for_customer') || app('business_settings')->where('type', 'email_verification')->first()->status == 0)
                                        ...
                                        <input type="text" id="email" name="email" value="{{old('email')}}" onfocus="this.placeholder = ''"
                                               >
                                        @error('email')
                                        <span class="text-danger" >{{ $message }}</span>
                                        @enderror
                                    @else
                                        ...
                                        <input type="email" id="email" name="email" value="{{old('email')}}" onfocus="this.placeholder = ''"
                                              >
                                        @error('email')
                                        <span class="text-danger" >{{ $message }}</span>
                                        @enderror
                                ...
                                <div class="col-md-6 mb-10">
                                    ...
                                    <input type="text" id="phone" onkeypress="return /[0-9_ ]/.test(event.key)" maxlength="10" name="phone" onfocus="this.placeholder = ''"
                                ...
                                <div class="position-relative">
                                    <input type="password" id="passInput" class="@error('password') is-invalid @enderror" name="password" >
                                    <span class="input-group-addon" style="position:absolute; top:8px; right:19px; font-size:20px;" role="button" title="veiw password" id="passBtn">
                                ...
                                <div class="col-md-6 mb-10">
                                    ...
                                    <input type="password" id="passInputConfirm" name="password_confirmation"  onfocus="this.placeholder = ''" autocomplete="new-password">
