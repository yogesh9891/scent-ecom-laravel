    ...
    <div class="checkout_v3_area">
        <div class="checkout_v3_left d-flex justify-content-end">
            <div class="checkout_v3_inner">
                                ...
                                <p>
                                    ...
                                    <span class="name_text text-nowrap">
                                            <a class="link_style" href="javascript:void(0)">
                                                ...
                                                <span id="shipping_methods" data-target="shipping_methods_{{$package_wise_shipping[$seller_id]['seller_id']}}">{{($package_wise_shipping[$seller_id]['shipping_cost'])}} via {{$package_wise_shipping[$seller_id]['shipping_method']}}   {{$package_wise_shipping[$seller_id]['shipping_time']}} =></span>
                                                @else
                                                {{($package_wise_shipping[$seller_id]['shipping_cost'])}} via {{$package_wise_shipping[$seller_id]['shipping_method']}}   {{$package_wise_shipping[$seller_id]['shipping_time']}}
                                                @endif
                                            </a>
                                    </span>
                                </p>
                                                ...
                                                @if($item->product->product->hasDeal)
                                                    @if($item->product->product->hasDeal->discount > 0)
                                                        @if($item->product->product->hasDeal->discount_type == 0)
                                                            ...
                                                            <span class="prise">₹ {{($item->product->selling_price)}}</span>
                                                        @else
                                                            <span class="prise_offer">₹ -{{($item->product->product->hasDeal->discount)}}</span>
                                                            <span class="prise">₹ {{($item->product->selling_price)}}</span>
                                                        @endif
                ...
                <div class="shiping_address_box checkout_form m-0" style="margin-top:0 !important;">
                    <div class="billing_address">
                        <h3 class="check_v3_title mb-25">{{__('defaultTheme.contact_information')}}</h3>
                        @if(auth()->check())
                        ...
                        <div class="shipping_delivery_div">
                            <h3 class="check_v3_title" style="margin-bottom: 20px"><span class="address_title">@if(!$delivery_info || $delivery_info && $delivery_info['delivery_type'] == 'home_delivery') {{__('shipping.shipping_address')}} @else {{__('common.billing_address')}} @endif</span> @if($shipping_address) <a href="javascript:void(0)" class="link_btn_design">{{__('common.edit')}}</a> @endif</h3>
                            ...
                            <div class="col-lg-12">
                                <div class="table-responsive">
                                    <table class="table table-borderless">
                                        <tr>
                                            <td class="headingtd_ckeckout"> {{__('common.name')}}</td>
                                            <td>: {{$shipping_address?$shipping_address->name:$user_name}}</td>
                                        </tr>
                                        <tr>
                                            <td class="headingtd_ckeckout"> {{__('common.address')}}</td>
                                            <td>: {{$shipping_address?$shipping_address->address:''}}</td>
                                        </tr>
                                        <tr>
                                            <td class="headingtd_ckeckout"> {{__('common.email')}}</td>
                                            <td>:  {{$shipping_address?$shipping_address->email:$user_email}}</td>
                                        </tr>
                                        <tr>
                                            <td class="headingtd_ckeckout"> {{__('common.phone')}}</td>
                                            <td>: {{$shipping_address?$shipping_address->phone:$user_phone}}</td>
                                        </tr>
                                        <tr>
                                            <td class="headingtd_ckeckout"> Pin Code</td>
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="form-group">
                                    ...
                                    <textarea cols="2" rows="2" name="note" id="note" placeholder="{{__('common.note')}}">{{session()->get('order_note')}}</textarea>
                                    ...
                                    @else
                                        <div id="next_step_btn_div">
                                            ...
                                            @else
                                                ...
                                                <button type="submit" class="btn_1 m-0 text-uppercase ">{{__('defaultTheme.continue_to_payment')}}fffff</button>
                                            @endif
                                        </div>
                                    @endif
        ...
        <div class="checkout_v3_right d-flex justify-content-start">
            <div class="order_sumery_box flex-fill">
                @if(!isModuleActive('MultiVendor'))
                      ...
                      <div class="singleVendor_product_lists">
                              ...
                              <div class="product_list_content">
                                  ...
                                  @endif
                                    <h5 class="d-flex align-items-center"><span
                                        class="product_count_text">{{$cart->qty}}<span>x</span></span>₹ {{($cart->price)}}</h5>
                                </div>
                            </div>
                        </div>
                        @if(Auth::user()  &&  Auth::user()->is_membership)
                          @php
                            ...
                            $subtotal += $cart->product->sku->vip_member_price * $cart->qty;
                          @endphp
                        @else
                          @php
                            $actual_price += $cart->total_price;
                      ...
                      <div class="singleVendor_product_lists">
                        <div class="singleVendor_product_list d-flex align-items-center">
                            ...
                            <div class="product_list_content">
                                ...
                                <h5 class="d-flex align-items-center"><span class="product_count_text" >{{$cart->qty}}<span>x</span></span>₹ {{($cart->price)}}</h5>
                            </div>
                        </div>
                      </div>
                      @php
                  ...
                  @endforeach
                @endif
                <h3 class="check_v3_title" style="margin-bottom: 20px">{{__('common.order_summary')}} </h3>
                <div class="subtotal_lists">
                        ...
                        @php
                            if(\Session::has('coupon_type')&&\Session::has('coupon_discount')){
                                ...
                                elseif($coupon_type == 4){
                                    $couponProducts = \Session::get('coupon_brands');
                                    if($coupon_discount_type == 0){
                                        foreach($couponProducts as  $key => $item){
                                            $cart = \App\Models\Cart::where('user_id',auth()->user()->id)->where('is_select',1)->where('product_type', 'product')->whereHas('product',function($query) use($item){
                                                $query->whereHas('product', function($q) use($item){
                    ...
                    <div class="total_amount d-flex align-items-center flex-wrap pb-5">
                        <div class="single_total_left flex-fill">
                            <span class="total_text">Cart Total</span>
                        </div>
                        <div class="single_total_right">
                            <span class="total_text"><span>₹ {{($total-$coupon)}}</span></span>
                        </div>
                    </div>
                </div>
            </div>
