    ...
    <div class="checkout_v3_area">
            ...
            <div class="checkout_v3_inner w-100">
                @if($free_shipping && !isModuleActive('MultiVendor'))
                    <div class="free_shipping_message">
                        {{-- <h5>Shipping charge free from <span>{{($free_shipping->minimum_shopping)}}</span></h5> --}}
                    </div>
                @endif
                            ...
                            {{-- {{dd($cart)}} --}}
                                @if($cart->product_type == 'product')
                                    @if(Auth::user()  &&  Auth::user()->is_membership)
                                        @php
                                            $subtotal += $cart->product->vip_member_price * $cart->qty;
                                        @endphp
                                    @elseif($cart->is_select == 1)
                                        @php
                                    ...
                                    <div class="single_checkout_shiped_product d-flex align-items-center">
                                        ...
                                        <div class="product_information d-flex align-items-center">
                                            <div class="thumb">
                                                ...
                                                @if(@$cart->product->product->product->product_type == 1)
                                                    @if(@$cart->product->product->product->thumbnail_image_source != null)
                                                    {{asset(asset_path(@$cart->product->product->product->thumbnail_image_source))}}
                                                    @else
                                                    {{asset(asset_path(@$cart->product->product->product->gallary_images[0]->images_source))}}
                                                    @endif
                                                @else
                                     ...
                                     @php  $price =$cart->product->selling_price;   @endphp
                                        <div class="offer_prise">
                                            {{-- {{dd($cart)}} --}}
                                            @if(Auth::user() && Auth::user()->is_membership  && ($cart->product->vip_status == 1))
                                                <span>{{$cart->product->vip_member_price}}</span>
                                                @php  $price =$cart->product->vip_member_price;  @endphp
                                            @elseif($cart->product->product->hasDeal)
                                                @if($cart->product->product->hasDeal->discount > 0)
                                                    @if($cart->product->product->hasDeal->discount_type == 0)
                                                    @php  $price =$cart->product->selling_price -(($cart->product->selling_price * $cart->product->product->hasDeal->discount)/100);   @endphp
                                                        <span class="prise_offer">₹ {{$price}} </span>
                                                        <span class="prise">₹ {{($cart->product->selling_price)}}</span>
                                                    @else
                                                     @php  $price =$cart->product->product->hasDeal->discount;   @endphp
                                                        <span class="prise_offer">₹ {{($cart->product->product->hasDeal->discount)}}</span>
                                                        <span class="prise">₹ {{($cart->product->selling_price)}}</span>
                                                    @endif
                                                @else
                                                    <span class="prise_main">₹ {{($cart->product->selling_price)}}</span>
                                                @endif
                                            @else
                                                @if(@$cart->product->product->hasDiscount == 'yes')

                                                    @if($cart->product->product->discount_type == 0)
                                                     @php  $price =$cart->product->selling_price -((($cart->product->selling_price)*($cart->product->product->discount)/100))   @endphp

                                                       <span class="prise_offer">₹ {{$cart->product->selling_price -((($cart->product->selling_price)*($cart->product->product->discount)/100)) }}</span> 
                                                        <span class="prise">₹ {{($cart->product->selling_price)}}</span>
                                                    @else
                                                     @php  $price =($cart->product->selling_price - $cart->product->product->discount)   @endphp

                                                        <span class="prise_offer">₹ {{($cart->product->selling_price - $cart->product->product->discount)}}</span>
                                                        <span class="prise">₹ {{($cart->product->selling_price)}}</span>
                                                    @endif
                                                @else
                                                    <span class="prise_main">₹ {{($cart->product->selling_price)}}</span>
                                        ...
                                        <div class="product_count">
                                            ...
                                            <div class="button-container">
                                                <button class="cart-qty-plus change_qty qtyChange" data-qty_id="#qty_{{$cart->id}}" data-change_amount="1" data-cart-id="{{$cart->id}}" data-price="{{$price}}" data-maximum_qty="#maximum_qty_{{$cart->id}}"
                                                ...
                                                <button class="cart-qty-minus change_qty qtyChange" data-qty_id="#qty_{{$cart->id}}" data-change_amount="1" data-cart-id="{{$cart->id}}"  data-price="{{$price}}"  data-maximum_qty="#maximum_qty_{{$cart->id}}"
                                        ...
                                        <div class="total_prise d-flex align-items-center">
                                            <span>₹ <span id="total_price_{{$cart->id}}">{{($cart->total_price)}}</span></span>
                                    ...
                                    @if($cart->product_type == 'product')
                                        ...
                                        <div class="single_checkout_shiped_product d-flex align-items-center">
                                            ...
                                            <div class="product_count">
                                                ...
                                                <input type="hidden" id="minimum_qty_{{$cart->id}}" value="{{$cart->product->product->product->minimum_order_qty}}">
                                                <div class="button-container">
                                                    <button class="cart-qty-plus change_qty" id="qtyChange" data-qty_id="#qty_{{$cart->id}}" data-change_amount="1" data-maximum_qty="#maximum_qty_{{$cart->id}}"
                                                         data-minimum_qty="#minimum_qty_{{$cart->id}}" data-product_stock="{{$cart->product->product_stock}}" data-stock_manage="{{$cart->product->product->stock_manage}}" type="button" value="+"><i class="ti-plus"></i></button>
                                                    <button class="cart-qty-minus change_qty" id="qtyChange" data-qty_id="#qty_{{$cart->id}}" data-change_amount="1" data-maximum_qty="#maximum_qty_{{$cart->id}}"
                                                         data-minimum_qty="#minimum_qty_{{$cart->id}}" data-product_stock="{{$cart->product->product_stock}}" data-stock_manage="{{$cart->product->product->stock_manage}}" type="button" value="-"><i class="ti-minus"></i></button>
                                                </div>
                                            </div>
                                            <div class="total_prise d-flex align-items-center">
                                            ...
                                            <div class="offer_prise">
                                                @if($cart->giftCard->hasDiscount())
                                                    ...
                                                    @else
                                                        <span class="prise_offer">-{{$cart->giftCard->discount}}</span>
                                                    @endif
                                                    <span class="prise">₹ {{($cart->giftCard->selling_price)}}</span>
                                                @else
                                                    <span class="prise_main">₹ {{($cart->giftCard->selling_price)}}</span>
                                                @endif
                                            </div>
        ...
        @endphp
        <div class="checkout_v3_right d-flex justify-content-start sticky-top">
            <div class="order_sumery_box flex-fill">
                ...
                <div class="subtotal_lists">
                    ...
                    <div class="single_total_list d-flex align-items-center flex-wrap">
                        <div class="single_total_left flex-fill">
                            <h4>{{__('common.shipping_charge')}} <a data-toggle="tooltip" data-placement="top" title="" data-original-title="Free shipping for orders more than 1,000 INR">
                    ...
                    {{-- <div class="single_total_list d-flex align-items-center flex-wrap">
                        <div class="single_total_left flex-fill">
                            <h4>{{__('common.vat')}}/{{__('common.tax')}}/{{__('gst.gst')}}</h4>
                        </div>
                        <div class="single_total_right">
                            <span>{{__('defaultTheme.calculated_at_next_step')}}</span>
                        </div>
                    ...
                    <div class="total_amount d-flex align-items-center flex-wrap">
                        <div class="single_total_left flex-fill">
                            <span class="total_text">Cart Total</span>
                        </div>
                        <div class="single_total_right">
                            <span class="total_text"><span>₹ {{($grand_total)}}</span></span>
                        </div>
                    </div>
                    <div class="check_v3_btns flex-wrap d-flex align-items-center mt-25 gap_10 ">
                        {{-- <div class="d-flex align-items-center gap_10 flex-fill">
