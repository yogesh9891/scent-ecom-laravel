                                ...
                                <div class="col-lg-6">
                                    ...
                                    <div class="delivery_des_div">
                                        <ul class="delivery_des_ul">
                                            ...
                                            <li>₹{{($cartItems[0]->shippingMethod->cost)}}</li>
                                            @if($addtional_charge > 0)
                                            <li>

                                                {{__('defaultTheme.addtional_shipping_charge')}} ₹: {{($addtional_charge)}}
                                            </li>
                                            @endif
                                                    ...
                                                    @if($seller->role->type == "admin")
                                                        ...
                                                        @php
                                                            ...
                                                            @foreach ($sameStateTaxes as $key => $sameStateTax)
                                                                ...
                                                                @endphp
                                                                <li>{{ $sameStateTax->name }}({{ $sameStateTax->tax_percentage }} %) : ₹ {{ ($gstAmount) }}</li>
                                                        ...
                                                        @else
                                                            ...
                                                            @foreach ($diffStateTaxes as $key => $diffStateTax)
                                                                ...
                                                                <input type="hidden" name="gst_amounts_package_{{ $index }}[]" value="{{ $gstAmount }}">
                                                                <li>{{ $diffStateTax->name }}({{ $diffStateTax->tax_percentage }} %) : ₹ {{ ($gstAmount) }}</li>
                                                            @endforeach
                                                        @endif
                                                    @else
                                        ...
                                        @else
                                            ...
                                            <div class="delivery_des_div">
                                                ...
                                                <ul class="delivery_des_ul">
                                                    ...
                                                    <input type="hidden" name="gst_amounts_package_{{ $index }}[]" value="{{ $gstAmount }}">
                                                    <li>{{ $flatTax->name }}({{ $flatTax->tax_percentage }} %) : ₹ {{ ($gstAmount) }}</li>
                                                </ul>
                                            </div>
                                        @endif
                                    @endif
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-2 single_product_price_div">
                                            ...
                                            <div class="price_with_dis">
                                                @if($product->hasDiscount())
                                                    ...
                                                    @else
                                                        <span class="offer_prise text-nowrap">₹ -{{($product->discount)}}</span>
                                                    @endif
                                                    <span class="curent_prise selling_original_price">₹ {{($product->selling_price)}}</span>
                                                @else
                                                <span class="curent_prise price_rate">₹ {{($product->selling_price)}}</span>
                                                @endif
                                            </div>
                                        ...
                                        <div class="col-md-3 d-flex tax_price_sec">
                                            <div class="price_tax_div">
                                                ...
                                                <p>{{__('common.tax')}} : 0 %</p>
                                                <p>{{__('defaultTheme.tax_amount')}} : ₹ {{(0)}}</p>
                                                <p>{{__('common.total')}}: ₹ {{($item->total_price)}}</p>
                                            </div>
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-3 d-flex tax_price_sec">
                                            <div class="price_tax_div">
                                                ...
                                                <p>{{__('common.tax')}} : @if($item->product->product->tax_type == 0) {{$item->product->product->tax}} % @else ₹{{($item->product->product->tax)}} @endif</p>
                                                <p>{{__('defaultTheme.tax_amount')}} : ₹{{(tax_count($item->price, $item->product->product->tax, $item->product->product->tax_type) * $item->qty)}}</p>
                                                @php
        ...
        <div class="order_information">
            ...
            <ul>
                ...
                <li>
                    ...
                    <p>{{__('common.subtotal')}}</p><span>₹ {{($subtotal)}}</span>
                </li>
                <li>
                    <input type="hidden" name="shipping_total" id="shipping_total" value="{{$shippingtotal}}">
                    <p>{{__('defaultTheme.shipping')}}</p><span>₹ {{($shippingtotal)}}</span>
                </li>
                <li>
                    @if(\Session::has('coupon_type')&&\Session::has('coupon_discount'))
                    ...
                    <p>{{__('common.coupon')}} {{__('common.discount')}}</p><strong id="coupon_delete">X</strong><span>₹ -{{($coupon)}}</span>
                    @else
                ...
                <li>
                    ...
                    <p>{{__('common.discount')}}</p><span>₹ -{{($subtotal - $actualtotal)}}</span>
                </li>
                <li>
                    <input type="hidden" name="tax_total" id="tax_total" value="{{$taxAmount}}">
                    <p>{{__('common.total')}} {{__('common.tax')}}</p><span>₹ {{($taxAmount)}}</span>
                </li>
                @if (file_exists(base_path().'/Modules/GST/'))
                    <li>
                        <input type="hidden" name="gst_tax_total" id="gst_tax_total" value="{{$gstAmountTotal}}">
                        <p>{{__('gst.total_gst')}}</p><span>₹ {{($gstAmountTotal)}}</span>
                    </li>
                @endif
            </ul>
    ...
    <div class="billing_address payment_option">
        ...
        <div class="form-row">
            <div class="col-md-12">
                ...
                <h4 class="pb-0 mb-0 wallet_balance">{{__('common.wallet_balance')}}: {{ (auth()->user()->CustomerCurrentWalletAmounts) }}</h4>
