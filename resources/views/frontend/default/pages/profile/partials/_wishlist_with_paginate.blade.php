  ...
  <div class="row ">
    @foreach($products as $product)
      @if($product->type =='product')
        <div class="col-lg-3 col-sm-6 col-md-6 single_product_item">
          <div class="single_product_list product_tricker">
            ...
            <div class="product_text">
              ...
              <div class="product_review_star d-flex justify-content-between align-items-center">
                <p>
                  @if(@$product->product->hasDeal)
                    ...
                    @if (@$product->product->product->product_type == 1)
                     ₹ {{(selling_price(@$product->product->skus->first()->selling_price,@$product->product->hasDeal->discount_type,@$product->product->hasDeal->discount))}}
                    @else
                        @if(selling_price(@$product->product->skus->min('selling_price'),@$product->product->hasDeal->discount_type,@$product->product->hasDeal->discount) === selling_price(@$product->product->skus->max('selling_price'),@$product->product->hasDeal->discount_type,@$product->product->hasDeal->discount))
                           ₹ {{(selling_price(@$product->product->skus->min('selling_price'),@$product->product->hasDeal->discount_type,@$product->product->hasDeal->discount))}}
                        @else
                           ₹ {{(selling_price(@$product->product->skus->min('selling_price'),@$product->product->hasDeal->discount_type,@$product->product->hasDeal->discount))}} - {{(selling_price($product->product->skus->max('selling_price'),@$product->product->hasDeal->discount_type,@$product->product->hasDeal->discount))}}
                        @endif
                    @endif
                  @elseif(Auth::user()  &&  Auth::user()->is_membership  && ($product->product->skus->first()->vip_status == 1))
                  <span class="">₹ {{(selling_price(@$product->product->skus->first()->vip_member_price ))}}</span>
                  @else
                    @if (@$product->product->product->product_type == 1)
                      @if(@$product->product->hasDiscount == 'yes')
                       ₹ {{(selling_price(@$product->product->skus->first()->selling_price,@$product->product->discount_type,@$product->product->discount))}}
                      @else
                       ₹ {{(@$product->product->skus->first()->selling_price)}}
                      @endif
                    @else
                      @if(@$product->product->hasDiscount == 'yes')
                        @if (selling_price(@$product->product->skus->min('selling_price'),@$product->product->discount_type,@$product->product->discount) === selling_price(@$product->product->skus->max('selling_price'),@$product->product->discount_type,@$product->product->discount))
                           ₹ {{(selling_price(@$product->product->skus->min('selling_price'),@$product->product->discount_type,$product->product->discount))}}
                        @else
                           ₹ {{(selling_price(@$product->product->skus->min('selling_price'),@$product->product->discount_type,$product->product->discount))}} - {{(selling_price(@$product->product->skus->max('selling_price'),@$product->product->discount_type,@$product->product->discount))}}
                        @endif
                      @else
                        @if (@$product->product->skus->min('selling_price') === @$product->product->skus->max('selling_price'))
                           ₹ {{(@$product->product->skus->min('selling_price'))}}
                        @else
                           ₹ {{(@$product->product->skus->min('selling_price'))}} - {{(@$product->product->skus->max('selling_price'))}}
                        @endif
                      @endif
                    @endif
                  @endif
                ...
                <div class="review_star_icon" style="display:unset">
                <a href="" class="removeWishlist deletbtnwishlist" data-id="{{ $product->id }}"> <i class="ti-trash"></i> </a>
                  <!-- @php
              ...
              @else
                @if(@$product->product->hasDiscount == 'yes')
                  <span class="price_off">
                  <div class="socal_icon">
                <!-- <a href="" class="addToCompareFromThumnail" data-producttype="1" data-seller="1" data-product-sku="3" data-product-id="3"> <i class="ti-exchange-vertical"></i> </a> -->
                <a href="" class="addToCartFromThumnail" data-producttype="1" data-seller="1" data-product-sku="3" data-base-price="3800" data-shipping-method="0" data-product-id="3" data-stock_manage="1" data-stock="200" data-min_qty="1"> <i class="ti-bag"></i> </a>
                <!-- <a href="" class="removeWishlist" data-id="28"> <i class="ti-trash"></i> </a> -->
              </div>
              ...
              <div class="product_text">
                  ...
                  <div class="product_review_star d-flex justify-content-between align-items-center">
                      @if(@$product->giftcard->hasDiscount())
                          ...
                          <p> ₹ {{(selling_price(@$product->giftcard->selling_price, @$product->giftcard->discount_type, @$product->giftcard->discount))}}</p>
                      @else
                          <p>₹ {{(@$product->giftcard->selling_price)}}</p>
                      @endif
                  ...
                  <div class="product_review_count d-flex justify-content-between align-items-center">
                      <span>
                          @if(@$product->giftcard->hasDiscount())
                             ...
                             ₹ {{(@$product->giftcard->selling_price)}}
                              @endif
                          @endif
                      </span>
