...
class CategoryController extends Controller
{
    ...
    { 
        ...
        if ($request->requestItemType == "gender") {
            $data['gender'] = $request->requestItem;
            $data['item'] = "gender";
        }
        // dd($data);
    ...
    {  
        ...
        if ($item == 'category') {
            ...
            if($category){
                ...
                // $data['CategoryList'][0] =Category::with('subCategories')->find($category_id);
                $data['CategoryList'] = $catRepo->allcategory();
                ...
                $attributeRepo = new AttributeRepository;
                  ...
                  $data['attributeLists'] = Attribute::with('values')->get();
        ...
        if($item == 'gender'){
            $brandRepo = new BrandRepository(new Brand());
            $gender_Arr = ['his','her','unisex'];
            if(in_array($slug,$gender_Arr)){
                $gender = $slug;
                $data['filter_name'] = $slug;
                $data['gender'] = $gender;
                $data['products'] = $this->filterService->filterProductGenderWise($gender, $sort_by, $paginate);
                $product_min_price = $this->filterService->filterProductMinPrice($data['products']->pluck('id')->toArray());
                $product_max_price = $this->filterService->filterProductMaxPrice($data['products']->pluck('id')->toArray());
                $product_min_price = $this->filterService->getConvertedMin($product_min_price);
                $product_max_price = $this->filterService->getConvertedMax($product_max_price);
               ...
               $data['brandList'] = Brand::get();
                $attributeRepo = new AttributeRepository;
                $data['attributeLists'] = Attribute::with('values')->get();
                $data['color'] = $attributeRepo->getColorAttributeForSpecificBrand($gender);
            }else{
                return abort(404);
            }
        }
        ...
        if($item == 'product'){
            ...
            $data['CategoryList'] = Category::whereRaw("id in ('". implode("','",$category_ids). "')")->where('parent_id',0)->take(10)->get();
            
            $data['brandList'] = Brand::whereRaw("id in ('". implode("','",$brand_ids). "')")->take(10)->get();
            $attribute_ids = ProductVariations::whereRaw("product_id in ('". implode("','",$main_product_ids)."')")->distinct()->pluck('attribute_id')->toArray();
            $data['attributeLists'] =  Attribute::with('values')->whereRaw("id in ('". implode("','", $attribute_ids). "')")->where('id','>',1)->take(1)->get();
            $data['color'] = Attribute::with('values')->whereRaw("id in ('". implode("','", $attribute_ids). "')")->where('id',1)->first();
        ...
        if ($item == 'tag') {
            ...
            $category_ids = CategoryProduct::whereRaw("product_id in ('". implode("','",$main_product_ids). "')")->distinct()->pluck('category_id')->toArray();
            $data['CategoryList'] = Category::whereRaw("id in ('". implode("','",$category_ids). "')")->take(20)->get();

            $data['brandList'] = Brand::whereRaw("id in ('". implode("','",$brand_ids). "')")->take(20)->get();
            $attribute_ids = ProductVariations::whereRaw("product_id in ('". implode("','",$main_product_ids). "')")->distinct()->pluck('attribute_id')->toArray();
            $data['attributeLists'] =  Attribute::with('values')->whereRaw("id in ('". implode("','", $attribute_ids). "')")->where('id','>',1)->take(2)->get();
            $data['color'] = Attribute::with('values')->whereRaw("id in ('". implode("','", $attribute_ids). "')")->first();
        ...
        if ($item == 'search') {
            ...
            $products = SellerProduct::activeSeller()->with('product')->select('seller_products.*')->join('products', function($q) use($main_product_ids,$slug){
                $q->on('seller_products.product_id','=','products.id')->whereRaw("product_id in ('". implode("','",$main_product_ids). "')");
            })->orWhere('products.product_name','LIKE', "%{$slug}%")->orWhere('seller_products.product_name', 'LIKE', "%{$slug}%")->take(100)->get();


            // $brandIds = Product::whereIn('id',$mainProducts)->distinct()->pluck('brand_id');
            $data['brandList'] = Brand::whereRaw("id in ('". implode("','",$main_product_ids)."')")->take(10)->get();
            $attribute_ids = ProductVariations::whereRaw("product_id in ('". implode("','",$main_product_ids). "')")->distinct()->pluck('attribute_id')->toArray();
            $data['attributeLists'] =  Attribute::with('values')->whereRaw("id in ('". implode("','", $attribute_ids). "')")->where('id','>',1)->take(2)->get();
            $data['color'] = Attribute::with('values')->whereRaw("id in ('". implode("','", $attribute_ids). "')")->first();
