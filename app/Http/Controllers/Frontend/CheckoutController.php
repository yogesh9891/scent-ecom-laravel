...
class CheckoutController extends Controller
{
    ...
    {   
        ...
        if($step == 'select_shipping'){
            ...
            $data['shiping_error'] = $errors;


            // $pickupDatas = $request->only('name','address','email','phone','country','state','city','postal_code');
            // //dd($pickupDatas);
            // try{
            ...
            //     $AreaCode = $result->AreaCode;
            //   }
            //     $result = $this->bluedartController->createPickup($pickupDatas);
            // } catch(\Exception $e){
            //     $errors = $e->getMessage();
            // }
            // $data['shiping_error'] = $errors;

            $wayBillService = $request->only('name','address','email','phone','country','state','city','postal_code');
            ...
            try{
                ...
                $result = $this->bluedartController->wayBillService($wayBillService,$data);
                // dd($result->GenerateWayBillResult->AWBNo);
                session()->put('wayBillNumber', $result->GenerateWayBillResult->AWBNo);
            } catch(\Exception $e){
                $errors = $e->getMessage();
            }
            $data['shiping_error'] = $errors;
        ...
        // return view(theme('pages.shipping_step'),$data);
          $address = $this->checkoutService->activeShippingAddress();
            ...
            if(isModuleActive('MultiVendor')){
                ...
                $selected_shipping_method = $this->checkoutService->selectedShippingMethod(decrypt(encrypt(1)));
        ...
        if($step == 'complete_order'){
            ...
            $wayBillNumber = session()->get('wayBillNumber');
            // dd($wayBillNumber);
    ...
    public function couponApply(CouponApplyRequest $request){
      ...
      $isMemberSip =   auth()->user()->is_membership;
        ...
        if(isset($coupon)){
            if(date('Y-m-d')>=$coupon->start_date && date('Y-m-d')<=$coupon->end_date){
                if($coupon->is_multiple_buy){
                        ...
                        if($isMemberSip){
                        if($coupon->for_member == 0) {
                                 return response()->json([
                                    'error' => 'This Coupon is not for Member'
                                ]);
                        }
                       } else {
                          ...
                          if($coupon->for_all == 0) {
                                 return response()->json([
                                    'error' => 'This Coupon is not for User'
                                ]);
                        }
                    ...
                    if($coupon->coupon_type == 1){
                        ...
                        $carts = Cart::where('user_id',auth()->user()->id)->where('is_select',1)->where('product_type', 'product')->pluck('product_id');
                       // dd($carts, "cart");
                        $products = CouponBrand::where('coupon_id', $coupon->id)->pluck('brand_id');
