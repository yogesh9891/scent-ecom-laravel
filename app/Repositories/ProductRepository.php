...
class ProductRepository
{
    ...
    {
        return $this->product::with('product', 'skus')->where('product_id', $id)->firstOrFail();
    }
    ...
    public function getActiveSellerProductBySlug($slug, $seller_slug = null)
    {
        if(isModuleActive('MultiVendor')){
            return $this->product::where('slug', $slug)->with('product.tags','related_sales.related_seller_products.seller','cross_sales.cross_seller_products.seller','up_sales.up_seller_products.seller', 'skus','seller','productSKU')->whereHas('seller', function($q) use ($seller_slug){
                return $q->where('slug', $seller_slug);
            })->activeSeller()->firstOrFail();
        }
        return $this->product::where('slug', $slug)->with('product.tags','related_sales.related_seller_products.seller','cross_sales.cross_seller_products.seller','up_sales.up_seller_products.seller', 'skus','seller','productSKU')->activeSeller()->firstOrFail();
    }
