...
class FilterRepository
{
    ...
    public function filterProductBlade($data, $sort_by, $paginate_by)
    {
        ...
        elseif($data['requestItemType'] == "tag"){
            ...
            // })->activeSeller();
            $products = SellerProduct::with('product')->activeSeller()->join('products', function($query) use($mainProducts){
                return $query->whereRaw("products.id in ('". implode("','",$mainProducts)."')")->join('product_variations', function($q3){
        ...
        //         $query->on('products.id','=','seller_products.product_id')->where('products.status', 1)->join('product_variations', function($q3){
        //             return $q3->on('products.id', '=', 'product_variations.product_id');
        //         });
        //     });
        // }
                ...
                $products = SellerProduct::with('product','categories')->where('status', 1)->whereHas('product', function($query) {
        ...
        foreach ($data['filterType'] as $key => $filter) {
            ...
            if ($filter['filterTypeId'] == "cat" && !empty($filter['filterTypeValue'])) {
                ...
                $products->whereHas('product', function($q1) use($typeVal){
                  $q1->whereHas('categories', function($q) use($typeVal){
                             return    $q->whereIn('category_id',$typeVal);
            ...
            if ($filter['filterTypeId'] == "brand" && !empty($filter['filterTypeValue'])) {
                ...
                // $products = $this->productThroughBrand($typeVal, $products, $requestType, $requestItem);
                       $products->whereHas('product', function($query) use($typeVal){
                return $query->whereIn('brand_id',$typeVal);
                   });
                $giftCards = collect();
            ...
            //     $giftCards = collect();
            // }
             if ($filter['filterTypeId'] == "gender" && !empty($filter['filterTypeValue'])) {
                $cat = $filter['filterTypeValue'];

                // $products = SellerProduct::with('skus', 'product')->where('status', 1)->whereHas('product', function ($query) use ($cat) {
                //     $query->whereIn('gender', $cat);
                // })->activeSeller();
                    $products->whereHas('product', function($query) use($cat){
                                          return $query->whereIn('gender',$cat);
                   });
                 // $products = $this->productThroughGender($cat, $products, $requestType, $requestItem);
                $giftCards = collect();
            }
    ...
    public function filterProductCategoryWise($category_id, $category_ids, $sort_by, $paginate_by)
    {
        ...
        $products = SellerProduct::with('skus', 'product')->where('seller_products.status', 1)->activeSeller()->select("seller_products.*")->join('products', function ($query) use ($category_ids, $category_id) {
            return $query->on('products.id', '=', 'seller_products.product_id')->where('products.status', 1)->join('category_product',function($q) use($category_id,$category_ids){
              ...
              if(!auth()->user() || (auth()->user() && auth()->user()->is_membership == 0)){
                  $products =  $products->whereHas('skus',function($q2) {
                        return $q2->where('vip_product',0);
            });
        }
  ...
  if(!auth()->user() || (auth()->user() && auth()->user()->is_membership == 0)){
    ...
    public function filterProductGenderWise($gender, $sort_by, $paginate_by)
    {
        $products = SellerProduct::with('skus', 'product')->where('seller_products.status', 1)->activeSeller()->select('seller_products.*')->join('products', function ($query) use ($gender) {
        return $query->on('seller_products.product_id', '=', 'products.id')->where('products.gender', $gender)->where('products.status', 1);
        });
          if(!auth()->user() || (auth()->user() && auth()->user()->is_membership == 0)){
                  $products =  $products->whereHas('skus',function($q2) {
                        return $q2->where('vip_product',0);
    ...
    public function filterCategoryBrandWise($brand_id)
    {
        ...
        })->where('categories.status', 1)->where('categories.parent_id','!=',0)->take(20)->get();
        return $categoryList;
    }
    ...
    public function filterBrandCategoryWise($category_id, $category_ids)
    {
        ...
        $brnadList = Brand::select('brands.*')->where('brands.status', 1)->join('products', function($q) use($category_ids, $category_id){
         ...
         $brnadList = Brand::select('brands.*')->where('brands.status', 1)->distinct('brands.id')->take(20)->get();
    ...
    public function productThroughParentCat($typeVal, $products)
    {
        ...
        //     $q->whereIn('category_id', $category_ids)->orWhereHas('subCategories', function($q1) use($category_ids){
        //         $q1->whereIn('id', $category_ids);
        //     });
        // });
        $products = $products->whereHas('categories', function ($q) use ($category_ids) {
    ...
    public function productThroughBrand($typeVal, $products, $requestType, $requestItem)
    {
        ...
        if ($requestType == "category") {            
            ...
            return $products;
                  ...
                  array_push($typeVal,$requestItem);
    ...
    public function productThroughAttribute($typeId, $typeVal, $products, $requestType, $requestItem)
    {
        ...
        if(count($typeVal) && count($typeId) > 0){
        ...
        if ($requestType ==  "category") {
            // $products = $products->whereHas('product', function ($q) use ($typeId, $typeVal, $requestItem) {
            //     $q->whereHas('categories', function($q1)use($requestItem){
            //         $q1->where('category_id',$requestItem)->orWhereHas('subCategories', function($q2) use($requestItem){
            //             $q2->where('parent_id', $requestItem);
            //         });
            ...
            //     });
            // })->join('product_variations', function($q3) use ($typeId, $typeVal){
            //     return $q3->on('products.id', '=', 'product_variations.product_id')->whereRaw("product_variations.attribute_id in ('". implode("','",[$typeId])."')")->whereRaw("product_variations.attribute_value_id in ('". implode("','",$typeVal)."')");
            // });

            // $products = $products->where('products.status', 1)->where('category_product.category_id', $requestItem)->join('product_variations', function($q3) use ($typeId, $typeVal){
            //     return $q3->on('products.id', '=', 'product_variations.product_id')->whereRaw("product_variations.attribute_id in ('". implode("','",[$typeId])."')")->whereRaw("product_variations.attribute_value_id in ('". implode("','",$typeVal)."')");
            // });
            ...
            //     $q->whereHas('variations', function ($query) use ($typeId, $typeVal) {
            //         $query->whereIn('attribute_id', [$typeId])->whereIn('attribute_value_id', $typeVal);
            //     })->whereIn('brand_id', [$requestItem])->where('products.status', 1);
            // });
            // $products = $products->where('products.status', 1)->whereRaw("products.brand_id in ('". implode("','",[$requestItem])."')")->join('product_variations', function($q2) use($typeId, $typeVal){
            //     return $q2->on('products.id', '=', 'product_variations.product_id')->whereRaw("product_variations.attribute_id in ('". implode("','",[$typeId])."')")->whereRaw("product_variations.attribute_value_id in ('". implode("','",$typeVal)."')");
            // });
            ...
            // $products = $products->where('products.status', 1)->join('product_variations', function($q) use ($typeId, $typeVal){
            //     return $q->on('products.id', '=', 'product_variations.product_id')->whereRaw("product_variations.attribute_id in ('". implode("','",[$typeId])."')")->whereRaw("product_variations.attribute_value_id in ('". implode("','",$typeVal)."')");
            // });
            $products = $products->where('products.status', 1)->whereRaw("product_variations.attribute_id in ('". implode("','",[$typeId])."')")->whereRaw("product_variations.attribute_value_id in ('". implode("','",$typeVal)."')");
        }
        return $products;
    ...
    private function productThroughRating($rating, $requestType, $requestItem, $products)
    {
      ...
      private function productThroughGender($gender,$products, $requestType, $requestItem)
    {
        if ($requestType ==  "category") {
