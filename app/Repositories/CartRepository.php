...
class CartRepository{
    ...
    public function store($data){
        ...
        if($data['type'] == 'product'){
            $sku = SellerProductSKU::with('product','productSku')->find($data['product_id']);
            if($sku && $sku->product->hasDeal){
                $price = selling_price(@$sku->selling_price,@$sku->product->hasDeal->discount_type,@$sku->product->hasDeal->discount,@$sku->vip_status,@$sku->vip_member_price);
            }else{
                if($sku && $sku->product->hasDiscount == 'yes'){
                    $price = selling_price(@$sku->selling_price,@$sku->product->discount_type,@$sku->product->discount,@$sku->vip_status,@$sku->vip_member_price);
