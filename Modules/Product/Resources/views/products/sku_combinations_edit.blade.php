...
// dd($query_1->variant_image_media);
	...
	@if(strlen($str) > 0)
			<tr class="variant text-center">
				...
				<td class="pt-25">
					<input  class="primary_input_field mt-30 selling_price" type="number" name="vip_member_price_sku[]" value="{{ ($query_1) ? @$query_1->vip_member_price : "0" }}" min="0" step="{{step_decimal()}}" class="form-control">
				</td>
				...
				<td class="text-center pt-25">
					<select class="primary_input_field mt-30 selling_price form-control" name="vip_status_sku[]" >
						...
						<option value="1" @if(!old('vip_status') || old('vip_status') == '1') selected @endif>yes</option>
						<option value="0" @if(!old('vip_status') || old('vip_status') == '0') selected @endif>no</option>
					</select>
				</td>
