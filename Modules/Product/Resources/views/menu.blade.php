    ...
    <li class="{{ $product_admin ?'mm-active' : '' }} sortable_li" data-position="{{ menuManagerCheck(1,12)->position }}" data-status="{{ menuManagerCheck(1,12)->status }}">
        ...
        <ul id="product_ul">
            @if (auth()->check() && auth()->user()->role->type == "superadmin" || auth()->check() && auth()->user()->role->type == "admin" || auth()->check() && auth()->user()->role->type == "staff")
                ...
                {{-- <li><a href="{{ route('product.gender.index') }}" @if (strpos(request()->getUri(),'gender-list') != false || strpos(request()->getUri(),'bulk-gender-upload') != false) class="active" @endif> Gender </a></li>
                
                @if (permissionCheck('product.gender.index') && menuManagerCheck(2,12,'product.gender.index')->status == 1)
                    <li data-position="{{ menuManagerCheck(2,12,'product.genders.index')->position }}">
                        <a href="{{ route('product.gender.index') }}" @if (strpos(request()->getUri(),'gender-list') != false || strpos(request()->getUri(),'bulk-gender-upload') != false) class="active" @endif> Gender </a>
                    </li>
                @endif --}}
