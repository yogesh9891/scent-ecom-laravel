   ...
   public function create(array $data)
    {   
        ...
        if ($data['product_type'] == 1) {
            ...
            $product_sku->vip_member_price = $data['vip_member_price'];
            $product_sku->vip_status = $data['vip_status'];
            $product_sku->vip_product = $data['vip_product'];
            $product_sku->gender  =  $data['gender'];
            $product_sku->new_tag  =  $data['new_tag'];
            // dd($product_sku->gender);
            $stock = 0;
            if (!isModuleActive('MultiVendor')) {
        ...
        if ($data['product_type'] == 2) {
            ...
            foreach ($data['track_sku'] as $key => $variant_sku) {
                ...
                $product_sku->vip_member_price = $data['vip_member_price_sku'][$key];
                $product_sku->vip_status = $data['vip_status_sku'][$key] ?? '0';
                $product_sku->vip_product = $data['vip_product_sku'][$key] ?? '0';
                $product_sku->new_tag = $data['new_tag_sku'][$key] ?? '0';
        ...
        if (auth()->user()->role->type == 'superadmin' || auth()->user()->role->type == 'admin' || auth()->user()->role->type == 'staff') {
            ...
            foreach ($product_skus as $key => $item) {
                ...
                SellerProductSKU::create([
                    ...
                    'vip_member_price' => $item->vip_member_price,
                    'vip_status' => $item->vip_status,
                    'vip_product' => $item->vip_product,
            ...
            if (!isModuleActive('MultiVendor')) {
                ...
                if($front_sku){
                    $front_sku->update([
                        ...
                        'vip_member_price' => $data['vip_member_price'],
            ...
            if(product_attribute_editable($product->id) === false){
                foreach ($data['track_sku'] as $key => $variant_sku) {
                    ...
                    if ($sku_exist == null) {
                        ...
                        $product_sku->gender  =  $data['gender'];
                        $product_sku->selling_price = $data['selling_price_sku'][$key];
                        $product_sku->vip_member_price = $data['vip_member_price_sku'][$key] ?? 0;
                        $product_sku->vip_status = $data['vip_status_sku'][$key] ?? 0;
                        $product_sku->vip_product = $data['vip_product_sku'][$key] ?? 0;
                        $product_sku->new_tag = $data['new_tag_sku'][$key] ?? 0;
                        $product_sku->additional_shipping = isset($data['additional_shipping']) ? $data['additional_shipping'] : 0;
                        ...
                        if(!isModuleActive("MultiVendor")){
                            ...
                            if($sellerProduct){
                                SellerProductSKU::create([
                                    ...
                                    'vip_member_price' => $product_sku->vip_member_price,
                                    'vip_status' => $product_sku->vip_status,
                                    'vip_product' => $product_sku->vip_product,
                                    'new_tag' => $product_sku->new_tag,
                                    'status' => 1,
                                    'user_id' => 1
                                ]);
                            }
                        ...
                        $sku_exist->vip_member_price = $data['vip_member_price_sku'][$key];
                        $sku_exist->vip_status = $data['vip_status_sku'][$key] ?? 0;
                        $sku_exist->vip_product = $data['vip_product_sku'][$key] ?? 0;
                        $sku_exist->new_tag = $data['new_tag_sku'][$key] ?? 0;
                        ...
                        if (!isModuleActive('MultiVendor')) {
                            ...
                            if($front_sku){
                                $front_sku->update([
                                    ...
                                    'vip_member_price' => $sku_exist->vip_member_price,
                                    'vip_status' => $sku_exist->vip_status,
                                    'vip_product' => $sku_exist->vip_product,
                                    'new_tag' => $sku_exist->new_tag,
                                ]);
                                if(isModuleActive('GoldPrice') && $data['auto_update_required']){
                                    if(@$front_sku->product->hasDeal){
                                        if(@$front_sku->product->hasDeal->discount > 0){
