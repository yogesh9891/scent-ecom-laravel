...
class GeneralSettingController extends Controller
{
    ...
    {  
        if ($request->has('status')) {
            $request->validate([
                "system_logo_name" => "required|string|max:30",
        ...
        if($request->has('system_logo_name')){
            $exsist_user = User::whereHas('role', function($q){
                return $q->where('type', 'seller');
            })->where('slug', $this->productSlug($request->system_logo_name))->first();
            if($exsist_user){
                $exsist_user->slug = $exsist_user->slug.'-'.$exsist_user->id;
                $exsist_user->save();
            }
            ...
            if($superadmin){
                $superadmin->slug = $this->productSlug($request->system_logo_name);
                $superadmin->save();
            }
        }
