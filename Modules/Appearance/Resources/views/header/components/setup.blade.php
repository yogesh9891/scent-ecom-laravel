                        ...
                        <div class="box_header">
                            <div class="main-title d-flex justify-content-between w-100">
                                <h3 class="mb-0 mr-30">
                                    ...
                                    {{__('appearance.offer_collection')}}
                                    @elseif($header->type == 'clients')
                                    {{__('appearance.clients')}}
                                    @endif
                        ...
                        <div class="box_header">
                            <div class="main-title d-flex justify-content-between w-100">
                                <h3 class="mb-0 mr-30">
                                    ...
                                    @elseif($header->type == 'offer_collection')
                                    {{__('appearance.offer_collection_section_setup')}}
                                    @elseif($header->type == 'clients')
                                    {{__('appearance.clients_section_setup')}}
                                    @endif
                                </h3>
                            </div>
                        </div>
...
@elseif($header->type == 'offer_collection')
@include('backEnd.partials._deleteModalForAjax',['item_name' => __('common.offer_collection'),'modal_id' =>
'deleteOfferCollectionModal','form_id' => 'offerCollection_delete_form','delete_item_id' => 'delete_offerCollection_id','dataDeleteBtn'
=>'offerCollectionDeleteBtn'])

@elseif($header->type == 'clients')
@include('backEnd.partials._deleteModalForAjax',['item_name' => __('common.clients'),'modal_id' =>
'deleteClientsModal','form_id' => 'clients_delete_form','delete_item_id' => 'delete_clients_id','dataDeleteBtn'
=>'clientsDeleteBtn'])
@endif
    ...
    (function($){
            ...
            $(document).ready(function() {
                ...
                $(document).on('submit', '#add_element_form', function(event) {
                    ...
                    if(header_type =='offer_collection'){
                        let offer_image = $('#offer_image')[0].files[0];
                        let name = form.name.value;
                        let data_type = form.url.value;
                        if(offer_image){
                            formData.append('offer_image',offer_image);
                        }
                        if(offer_image && name != '' && data_type != null){
                            ...
                            if(!offer_image){
                                toastr.error("{{__('appearance.offer_image_required')}}", "{{__('common.error')}}");
                            }
                            if(data_type == 'url' && $('#url').val() != ''){
                                toastr.error("{{__('appearance.url_is_required')}}", "{{__('common.error')}}");
                            }
                            if(data_type == null){
                                toastr.error("{{__('appearance.offer_image_is_required')}}", "{{__('common.error')}}");
                            }
                            $('#pre-loader').addClass('d-none');
                        }
                    }
                     
                    if(header_type =='clients'){
                        let clients_image = $('#clients_image')[0].files[0];
                        let name = form.name.value;
                        // let data_type = form.data_type.value;
                        if(clients_image){
                            formData.append('clients_image',clients_image);
                        }
                        if(clients_image && name != ''){
                            $.ajax({
                                url: "{{ route('appearance.header.setup.add-element') }}",
                                type: "POST",
                                cache: false,
                            ...
                            if(!offer_image){
                                toastr.error("{{__('appearance.clients_image_required')}}", "{{__('common.error')}}");
                            }
                            if(data_type == 'url' && $('#url').val() != ''){
                                toastr.error("{{__('appearance.url_is_required')}}", "{{__('common.error')}}");
                            }
                            if(data_type == null){
                                toastr.error("{{__('appearance.clients_image_is_required')}}", "{{__('common.error')}}");
                            }
                            $('#pre-loader').addClass('d-none');
                        }
                    }
                ...
                $(document).on('submit', '.element_edit_offer_form', function(event) {
                    event.preventDefault();
                    $('#pre-loader').removeClass('d-none');
                    var formElement = $(this).serializeArray()
                    var formData = new FormData();
                    ...
                    if(header_type == 'offer_collection'){
                        let offer_image = $(this)[0][7].files[0];
                        if(offer_image){
                            formData.append('offer_image',offer_image);
                        }
                    }
                    ...
                    if(header_type == 'clients'){
                        let clients_image = $(this)[0][8].files[0];
                        if(clients_image){
                            formData.append('clients_image',clients_image);
                        }
                    }
                ...
                $(document).on('submit', '#slider_delete_form', function(event){
                 ...
                 $(document).on('submit', '#offerCollection_delete_form', function(event){
                    event.preventDefault();
                    $('#pre-loader').removeClass('d-none');
                    $('#deleteOfferCollectionModal').modal('hide');
                    let id = $('#delete_OfferCollection_id').val();
                    let data = {
                        'id' : id,
                        '_token' : '{{ csrf_token() }}',
                        'header_id':'{{$header->id}}'
                ...
                $(document).on('click', '.offerCollection_delete_btn', function(event){
                    event.preventDefault();
                    let id = $(this).data('id');
                    if(id != null){
                        var asd = $('#delete_offerCollection_id').val(id);
                        var fgh= $('#deleteOfferCollectionModal').modal('show');
                    }else{
                        toastr.error('{{ __("common.error_message") }}')
                    }
                });
                ...
                $(document).on('change', '#offer_image', function(event){
                    getFileName($(this).val(),'#banner_image_file');
                    imageChangeWithFile($(this)[0],'#MetaImgDiv');
                });
                ...
                $(document).on('change', '.offerCollection_image_edit', function(event){
                    let img_id = $(this).data('img_id');
                    let name_id = $(this).data('name_id');
                    getFileName($(this).val(),name_id);
                    imageChangeWithFile($(this)[0],img_id);
                ...
                $(document).on('change', '#offerCollection_for', function(event){
                    let data_type = $('#offerCollection_for').val();
                    ...
                    if(data_type != null){
                        ...
                        $.post("{{route('appearance.header.get-slider-type-data')}}", data, function(response){
                            $('#pre-loader').addClass('d-none');
                            $('#offerCollection_for_data_div').html(response);
                            $('.offerCollection_for_drop').niceSelect();
                            dynamicSelect2WithAjax(".product", "{{url('/products/seller-products/get-by-ajax')}}", "GET");
                            dynamicSelect2WithAjax(".category", "{{url('/products/get-category-data')}}", "GET");
                            dynamicSelect2WithAjax(".offerCollection_for_brand", "{{route('product.brands.get-by-ajax')}}", "GET");
                            dynamicSelect2WithAjax(".offerCollection_for_tag", "{{url('/setup/tags/get-by-ajax')}}", "GET");
                        });
                    }
                });
                ...
                function offerCollection_form_reset(){

                    $('#createOfferCollectionImgDiv').html(
                        `<img id="MetaImgDiv"
                        src="{{ showImage('backend/img/default.png') }}" alt="">`
                    );

                    $('#offerCollectionImgFileDiv').html(
                        `<div class="primary_input mb-25">
                            <label class="primary_input_label" for="">{{ __('appearance.offerCollection_image') }} (1920 x 500) <span class="text-danger">*</span></label>
                            <div class="primary_file_uploader">
                                <input class="primary-input" type="text" id="banner_image_file"
                                    placeholder="{{__('common.browse')}} {{__('common.image')}}" readonly="">
                                <button class="" type="button">
                    ...
                    $('#offerCollectionr_for_data_div').empty();
                    $('#offerCollection_data_type_div').html(

                        `
                        <div class="primary_input mb-25">
                            <label class="primary_input_label" for="">{{ __('appearance.offerCollection_for') }}</label>
                            <select name="data_type" id="offerCollection_for" class="primary_select mb-15">
                                <option value="" selected disabled>Select One</option>
                                <option value="product">For Product</option>
                                <option value="category">For Category</option>
                                <option value="brand">For Brand</option>
                    ...
                    $('#offerCollection_for').niceSelect();
                }
                ...
                function reloadWithData(response){
                    ...
                    $('.edit_offerCollection_drop').niceSelect();
                    dynamicSelect2WithAjax(".product", "{{url('/products/seller-products/get-by-ajax')}}", "GET");
                    dynamicSelect2WithAjax(".category", "{{url('/products/get-category-data')}}", "GET");
                    dynamicSelect2WithAjax(".slider_brand", "{{route('product.brands.get-by-ajax')}}", "GET");
                    dynamicSelect2WithAjax(".offerCollection_brand", "{{route('product.brands.get-by-ajax')}}", "GET");
                    dynamicSelect2WithAjax(".slider_tag", "{{url('/setup/tags/get-by-ajax')}}", "GET");
                }
                dynamicSelect2WithAjax(".product", "{{url('/products/seller-products/get-by-ajax')}}", "GET");
                dynamicSelect2WithAjax(".category", "{{url('/products/get-category-data')}}", "GET");
                ...
                dynamicSelect2WithAjax(".offerCollection_brand", "{{route('product.brands.get-by-ajax')}}", "GET");
                dynamicSelect2WithAjax(".slider_tag", "{{url('/setup/tags/get-by-ajax')}}", "GET");
                dynamicSelect2WithAjax(".offerCollection_tag", "{{url('/setup/tags/get-by-ajax')}}", "GET");
            });
        })(jQuery);
