                ...
                <div class="box_header_right">
                    <div class="float-lg-right float-none pos_tab_btn justify-content-end">
                        <ul class="nav" role="tablist">
                            ...
                            <li class="nav-item">
                                <a class="nav-link" href="#vip_customer" role="tab" data-toggle="tab"
                                    id="1" aria-selected="true">{{ __('vip customer') }}</a>
                            </li>

                            <li class="nav-item">
                                <a class="nav-link dis" style="display: none;" href="#pending_customer" role="tab" data-toggle="tab"
                                    id="1" aria-selected="true">{{ __('pending customer') }}</a>
                            </li>
            ...
            <div class="col-xl-12">
                <div class="white_box_30px mb_30">
                    <div class="tab-content">
                        ...
                        <div role="tabpanel" class="tab-pane fade" id="vip_customer">
                            <div class="box_header common_table_header ">
                                <div class="main-title d-md-flex">
                                    <h3 class="mb-0 mr-30 mb_xs_15px mb_sm_20px">{{__('vip customer')}}</h3>
                                </div>
                            </div>
                            <div class="QA_section QA_section_heading_custom check_box_table">
                                <div class="QA_table">
                                    <!-- table-responsive -->
                                    <div class="">
                                        @include('customer::customers.components.is_membership_lists')
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div role="tabpanel" class="tab-pane fade" id="pending_customer">
                            <div class="box_header common_table_header ">
                                <div class="main-title d-md-flex">
                                    <h3 class="mb-0 mr-30 mb_xs_15px mb_sm_20px">{{__('pending customer')}}</h3>
                                </div>
                            </div>
                            <div class="QA_section QA_section_heading_custom check_box_table">
                                <div class="QA_table">
                                    <!-- table-responsive -->
                                    <div class="">
                                        @include('customer::customers.components.pending_lists')
                                    </div>
                                </div>
                            </div>
                        </div>
...
<script type="text/javascript">
    (function($){
            ...
            $(document).ready(function(){
                ...
                vipCustomerDataTable();
                pendingCustomerDataTable();
                ...
                $(document).on('change', '.update_active_status', function(event){
                 ...
                 $(document).on('change', '.update_is_member_status', function(event){
                    let id = $(this).data('id');
                    let is_membership = 0;
                    alert($(this).prop('checked'));
                    if($(this).prop('checked')){
                        console.log('zxxzcxzxczxCZXCZXCZX')
                        is_membership = 1;
                    }
                    else{
                        is_membership = 0;
                    ...
                    $.post('{{ route('customer.update_is_member_status') }}', {_token:'{{ csrf_token() }}', id:id, is_membership:is_membership}, function(data){
                        if(data == 1){
                            toastr.success("{{__('common.updated_successfully')}}","{{__('common.success')}}");
                            activeCustomerDataTable();
                            inactiveCustomerDataTable();
                ...
                function vipCustomerDataTable(){
                    $('#vipCustomerTable').DataTable({
                        processing: true,
                        serverSide: true,
                        stateSave: true,
                        "ajax": ( {
                            url: "{{ route('cusotmer.list.get-data') }}" + '?table=vip_customer'
                        }),
                        "initComplete":function(json){
                        ...
                        columns: [
                            ...
                            { data: 'email', name: 'email' },
                            { data: 'phone', name: 'phone' },
                            { data: 'is_membership', name: 'is_membership' },
                            { data: 'wallet_balance', name: 'wallet_balance' },
                            { data: 'orders', name: 'orders' },
                            { data: 'action', name: 'action' }
                ...
                function pendingCustomerDataTable(){
                    $('#pendingCustomerTable').DataTable({
                        processing: true,
                        serverSide: true,
                        stateSave: true,
                        "ajax": ( {
                            url: "{{ route('cusotmer.list.get-data') }}" + '?table=pending_customer'
                        }),
                        "initComplete":function(json){
