...
class CustomerController extends Controller
{
    ...
    public function customer_index_get_data(){
        if(isset($_GET['table'])){
            ...
            elseif($table == 'vip_customer'){
                $customer = $this->customerService->getAll()->where('is_membership', 1);
            }
            elseif($table == 'pending_customer'){
                $customer = $this->customerService->getAll()->where('referral_code', '!=',"")->where('is_membership', 0);
            }
            ...
            ->addColumn('status', function($customer){
             ...
             ->addColumn('is_membership', function($customer){
                return view('customer::customers.components._is_membership_td',compact('customer'));
            ...
            ->addColumn('orders', function($customer){
             ...
             ->addColumn('member', function($customer){
                 return view('customer::customers.components._referal_code',compact('customer'));
            })
    ...
    public function update_is_member_status(Request $request)
    {
        try {
            $userRepo = new UserRepository;
            $userRepo->memberUpdate($request->all());
            LogActivity::successLog('customer update member status');
            return 1;
        } catch (\Exception $e) {
            LogActivity::errorLog($e->getMessage());
            return 0;
